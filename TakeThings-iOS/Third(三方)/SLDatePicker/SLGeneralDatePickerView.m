...
#pragma mark - Getter
- (NSString *)date {
    NSString *dateString = [NSString stringWithFormat:@"%04ld-%02ld-%02ld", (long)self.selectedYear, self.selectedMonth, self.selectedDay];
    
    if (SLDatePickerModeYearAndMonth == self.datePickerMode) {
        dateString = [NSString stringWithFormat:@"%04ld-%02ld", (long)self.selectedYear, self.selectedMonth];
    } else if (SLDatePickerModeYear == self.datePickerMode) {
        dateString = [NSString stringWithFormat:@"%04ld", (long)self.selectedYear];
    }
    NSLog(@"日期%@",dateString);
    return dateString;
}
