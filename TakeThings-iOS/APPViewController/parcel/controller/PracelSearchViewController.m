...
@interface PracelSearchViewController ()
...
@implementation PracelSearchViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [Unity getColor:@"aa112d"];
    self.listArray = [[[NSUserDefaults standardUserDefaults] objectForKey:@"pracelSearch"] mutableCopy];
    [self createUI];
...
- (UITextField *)searchText{
    if (_searchText == nil) {
        ...
        _searchText.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"直接搜索包裹编号/运单编号" attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13],NSForegroundColorAttributeName:[Unity getColor:@"#d58896"]}]; ///新的实现
    }
    return _searchText;
}
...
//    NSLog(@"%@",[self.listArray class]);
    ...
    [[NSUserDefaults standardUserDefaults] setObject:self.listArray forKey:@"pracelSearch"];
}
- (void)searchClick{
...
- (void)push:(NSString *)keyWord{
    SearchResultsViewController * svc = [[SearchResultsViewController alloc]init];
    svc.keyWord = keyWord;
