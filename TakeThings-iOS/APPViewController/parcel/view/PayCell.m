...
#import "PayCell.h"
@interface PayCell()<UITextFieldDelegate>
...
- (void)bankNumClick:(UITextField *)textField{
    [self.delegate inputBankNum:textField.text];
}
- (void)idNumClick:(UITextField *)textField{
    [self.delegate inputIdNum:textField.text];
}
- (void)mobileNumClick:(UITextField *)textField{
    [self.delegate inputMobileNum:textField.text];
}
- (void)codeTextClick:(UITextField *)textField{
    [self.delegate inputCodeNum:textField.text];
}
- (void)nameClick:(UITextField *)textField{
    [self.delegate inputName:textField.text];
}

- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
//    NSLog(@"选择银行名称");
    [self.delegate seleteBankName];
    return NO;
}
- (void)requestCode{
    NSDictionary * info = [[NSUserDefaults standardUserDefaults] objectForKey:@"userInfo"];
    NSDictionary * dic = @{@"login_token":info[@"token"],@"user":info[@"w_email"],@"bid_id":self.order_id,@"payment_method":@"DEBIT_CARD",@"payer_name":self.nameText.text,@"payer_phone":self.mobileNumText.text,@"citizen_id":self.idNumText.text,@"bank_code":self.bankCode,@"bank_number":self.bankNumText.text,@"os":@"1"};
    NSLog(@"%@",dic);
    [Unity showanimate];
    NSString * url;
    if (self.isType) {
    ...
    [GZMrequest postWithURLString:url parameters:dic success:^(NSDictionary *data) {
        ...
        if ([[data[@"meta"][@"ret_code"] stringValue] isEqualToString:@"0000"]) {
           [self startTime];
            [self.delegate paymentNuf_id:data[@"paymentumf_id"]];
        }else{
