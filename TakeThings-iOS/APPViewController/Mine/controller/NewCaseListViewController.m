...
#import "PracelSearchViewController.h"
#import "CWScrollView.h"
#import "CWTableViewInfo.h"
#import "UIViewController+CWLateralSlide.h"
#import "PrecelScreenViewController.h"
@interface NewCaseListViewController ()<UIScrollViewDelegate,PrecelScreenDelegate>
{
...
@implementation NewCaseListViewController
...
- (void)viewDidLoad {
    ...
    [self.navigationItem setTitle:@"我的包裹"];
...
- (void)setupChildViewController {
    ...
    NewOneViewController *oneVC = [[NewOneViewController alloc] init];
    [self addChildViewController:oneVC];
    
    NewTwoViewController *twoVC = [[NewTwoViewController alloc] init];
    [self addChildViewController:twoVC];
    
    NewThreeViewController * sVC = [[NewThreeViewController alloc]init];
...
- (UIButton *)bidBtn{
    if (!_bidBtn) {
        ...
        [_bidBtn setTitle:@"已发出" forState:UIControlStateNormal];
        ...
        CGFloat w = [Unity widthOfString:@"已发出" OfFontSize:15 OfHeight:2];
...
- (void)searchClick{
    PracelSearchViewController * pvc = [[PracelSearchViewController alloc]init];
    pvc.tag = self.tap;
    [self.navigationController pushViewController:pvc animated:YES];
}
- (void)defaultAnimationFromRight:(NSInteger)tap{
    PrecelScreenViewController *vc = [[PrecelScreenViewController alloc] init];
    vc.delegate = self;
    vc.tap = tap;
    if (tap == 0) {
        vc.timeP = oneIndex;
...
- (void)screenTime:(NSInteger)time WithListIndex:(NSInteger)listIndex WithStatusIndex:(NSInteger)statusIndex{
    ...
    NSDictionary * dic = @{@"time":[NSString stringWithFormat:@"%ld",(long)time],@"status":[NSString stringWithFormat:@"%ld",(long)statusIndex]};
    NSString * str = [NSString stringWithFormat:@"precelList%ld",(long)listIndex];
    NSLog(@"%@",str);
