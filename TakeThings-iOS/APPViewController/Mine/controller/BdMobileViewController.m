...
@interface BdMobileViewController ()
{
    NSString * mobile;
...
@implementation BdMobileViewController
...
- (UILabel *)mobileL{
    if (!_mobileL) {
        _mobileL = [[UILabel alloc]initWithFrame:CGRectMake([Unity countcoordinatesW:10], [Unity countcoordinatesH:15], SCREEN_WIDTH-[Unity countcoordinatesW:20], [Unity countcoordinatesH:20])];
        _mobileL.text = [NSString stringWithFormat:@"您正在更换手机号：%@",[Unity editMobile:mobile]];
...
- (UILabel * )validationL{
    if (!_validationL) {
        _validationL = [[UILabel alloc]initWithFrame:CGRectMake(_mobileL.left, _mobileL.bottom+[Unity countcoordinatesH:15], 100, [Unity countcoordinatesH:20])];
...
- (UIButton *)boundBtn{
    if (_boundBtn == nil) {
        ...
        [_boundBtn setTitle:@"下一步" forState:UIControlStateNormal];
...
- (void)nextClick{
    ...
    NSLog(@"%@",params);
    [GZMrequest postWithURLString:[GZMUrl get_oldMobile_url] parameters:params success:^(NSDictionary *data) {
        NSLog(@"修改绑定手机——原始手机验证%@",data);
        [self.aAnimation stopAround];
        if ([[data objectForKey:@"code"]isEqualToString:@"success"]) {
            ReplaceMobileViewController * rvc = [[ReplaceMobileViewController alloc]init];
