...
@interface ANotBidViewController ()<UITableViewDelegate,UITableViewDataSource,NotBidCellDelegate,NoDataDelegate>
{
...
@implementation ANotBidViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NotBidCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([NotBidCell class])];
    if (cell == nil) {
        cell = [[NotBidCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([NotBidCell class])];
    }
...
- (void)deleteClick{
    ...
    NSDictionary *params = @{@"customer":userInfo[@"member_id"],@"ids":idsStr,@"area":@"a"};
...
//                [dic setObject:array[i][@"w_tbsl"] forKey:@"w_tbsl"];
                ...
                if ([array[i][@"w_ykj"] intValue] == 2) {
                    [dic setObject:@"2" forKey:@"w_ykj"];
                }else{
                    [dic setObject:@"" forKey:@"w_ykj"];
                }
                [dic setObject:@"0" forKey:@"isSelect"];
...
//                [dic setObject:data[@"data"][i][@"w_tbsl"] forKey:@"w_tbsl"];
                ...
                if ([data[@"data"][i][@"w_ykj"] intValue] == 2) {
                    [dic setObject:@"2" forKey:@"w_ykj"];
                }else{
                    [dic setObject:@"" forKey:@"w_ykj"];
                }
...
- (void)requestOrderlist{
    NSDictionary * userInfo = [[NSUserDefaults standardUserDefaults]objectForKey:@"userInfo"];
    NSDictionary * dic = @{@"area":@"a",@"customer":userInfo[@"member_id"],@"status":@"-1",@"page":@"1"};
    [GZMrequest getWithURLString:[GZMUrl get_orderlist_url] parameters:dic success:^(NSDictionary *data) {
        NSLog(@"B区为的表%@",data);
        if ([[data objectForKey:@"code"]isEqualToString:@"success"]) {
