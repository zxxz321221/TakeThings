...
@interface BidViewController ()<UITableViewDelegate,UITableViewDataSource,BidCellDelegate,DrainIntoViewDelegate,NoDataDelegate>
{
...
- (void)requestOrderlist:(NSInteger )page{
    ...
    NSDictionary * dic = @{@"area":@"b",@"customer":userInfo[@"member_id"],@"status":@"2",@"page":[NSString stringWithFormat:@"%ld",(long)page]};
...
- (void)queue{
    ...
    NSDictionary *params = @{@"customer":userInfo[@"member_id"],@"item":self.listArray[index][@"id"]};
    [GZMrequest postWithURLString:[GZMUrl get_queueCase_url]parameters:params success:^(NSDictionary *data) {
