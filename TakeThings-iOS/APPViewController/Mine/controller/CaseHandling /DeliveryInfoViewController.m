...
@interface DeliveryInfoViewController ()<UITableViewDelegate,UITableViewDataSource,PackagingCellDelegate,deliveryInfoCellDelegate,AddressViewDelegate>
{
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    if (section == 0) {
        ...
        if ([self.listArr[0][@"w_cc"]isEqualToString:@"0"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        deliveryInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([deliveryInfoCell class])];
        if (cell == nil) {
            cell = [[deliveryInfoCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([deliveryInfoCell class])];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        [cell configWithData:self.listArr[indexPath.row]];
...
- (void)confirmClick{
    ...
    if (![self.listArr[0][@"w_cc"] isEqualToString:@"0"]) {
//        dic = @{@"agent":self.listArr[0][@"numId"],@"cost":self.placeDic[@"0"],@"customer":userInfo[@"member_id"],@"address":addressId,@"transport":@"3",@"weight":self.listArr[0][@"w_kgs"],@"enhance":str1,@"insure":str2,@"safe":str3};
        transport = @"3";
    }
    NSString * agent = @"";
    NSString * cost = @"";
    ...
    for (int i=0; i<self.listArr.count; i++) {
        if ([agent isEqualToString:@""]) {
            agent = self.listArr[i][@"numId"];
        }else{
            agent = [agent stringByAppendingString:[NSString stringWithFormat:@",%@",self.listArr[i][@"numId"]]];
        }
        if ([cost isEqualToString:@""]) {
            cost = self.placeDic[[NSString stringWithFormat:@"%d",i]];
        }else{
            cost = [cost stringByAppendingString:[NSString stringWithFormat:@",%@",self.placeDic[[NSString stringWithFormat:@"%d",i]]]];
        }

        weight = weight + [self.listArr[i][@"w_kgs"] floatValue];

    }
    dic = @{@"agent":agent,@"cost":cost,@"customer":userInfo[@"member_id"],@"address":addressId,@"transport":transport,@"weight":[NSString stringWithFormat:@"%.2f",weight],@"enhance":str1,@"insure":str2,@"safe":str3};
    ...
    [GZMrequest postWithURLString:[GZMUrl get_send_url] parameters:dic success:^(NSDictionary *data) {
        NSLog(@"确认发货 %@",data);
...
//校验发货按钮是否可点击
- (void)checkInfo{
    if (self.listArr.count != self.placeDic.count || addressId == nil) {
        ...
        if ([[data objectForKey:@"code"]isEqualToString:@"success"]) {
            if ([data[@"data"] count] == 0) {
                ...
                for (int i=0; i<[data[@"data"] count]; i++) {
                    [self.addressArr addObject:data[@"data"][i]];
                }
                for (int i=0; i<self.addressArr.count; i++) {
                    if ([self.addressArr[i][@"if_default"]isEqualToString:@"1"]) {
                ...
                if (self.listArr.count>0) {
                       ...
                       [self.placeDic setObject:[NSString stringWithFormat:@"%.2f",[dic[@"w_jbj_jp"] doubleValue]+[dic[@"w_sj_jp"] doubleValue]] forKey:[NSString stringWithFormat:@"%d",0]];
                         [self checkInfo];
                   }
            }
...
- (void)allSeletedClick:(UIButton *)btn{
    ...
    float kg=0.0;
    for (int i=0; i<self.listArr.count; i++) {
        kg=kg+[self.listArr[i][@"w_kgs"] floatValue];
    }
    NSInteger yf=0.0;
    if (btnIndex ==0) {
