...
@interface HaitaoListCell2()
{
    BOOL isSend;//yes通知发货   no包裹详情
}
...
@property (nonatomic , strong) UIButton * oldBtn;//原始页面 详情
@property (nonatomic , strong) UIButton * detailBtn;//详情  通知发货
@end
@implementation HaitaoListCell2
...
- (UIView *)backView{
    if (!_backView) {
        ...
        [_backView addSubview:self.detailBtn];
...
- (UIButton *)detailBtn{
    if (!_detailBtn) {
        _detailBtn = [[UIButton alloc]initWithFrame:CGRectMake(_backView.width-[Unity countcoordinatesW:80], _line2.bottom+[Unity countcoordinatesH:10], [Unity countcoordinatesW:70], [Unity countcoordinatesH:30])];
        [_detailBtn addTarget:self action:@selector(detailClick:) forControlEvents:UIControlEventTouchUpInside];
        [_detailBtn setTitle:@"委托详情" forState:UIControlStateNormal];
        [_detailBtn setTitle:@"通知发货" forState:UIControlStateSelected];
        [_detailBtn setTitleColor:[Unity getColor:@"aa112d"] forState:UIControlStateNormal];
        [_detailBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        _detailBtn.layer.cornerRadius = _detailBtn.height/2;
        _detailBtn.layer.borderWidth =1;
        _detailBtn.layer.borderColor = [Unity getColor:@"aa112d"].CGColor;
...
- (UIButton *)oldBtn{
    if (!_oldBtn) {
        _oldBtn = [[UIButton alloc]initWithFrame:CGRectMake(_backView.width-[Unity countcoordinatesW:160], _line2.bottom+[Unity countcoordinatesH:10], [Unity countcoordinatesW:70], [Unity countcoordinatesH:30])];
        ...
        [_oldBtn setTitle:@"委托详情" forState:UIControlStateSelected];
        [_oldBtn setTitleColor:LabelColor3 forState:UIControlStateNormal];
        [_oldBtn setTitleColor:[Unity getColor:@"aa112d"] forState:UIControlStateSelected];
...
- (void)detailClick:(UIButton *)btn{
    if (btn.selected) {//通知发货
        if (isSend) {//fahuo
            [self.delegate send:self];
        }else{//包裹详情
            [self.delegate bgDetail:self];
        }
...
- (void)oldClick:(UIButton *)btn{
    if (btn.selected) {//委托详情
        [self.delegate detail:self];
    }else{//原始页面
        [self.delegate oldPage:self];
    }
}
- (void)configWithData:(NSDictionary *)dic{
    ...
    if ([dic[@"order_bid_status_id"] intValue] == 130) {
        isSend = YES;
        self.detailBtn.selected = YES;
        self.detailBtn.backgroundColor = [Unity getColor:@"aa112d"];
        self.oldBtn.selected = YES;
        self.oldBtn.layer.borderColor = [Unity getColor:@"aa112d"].CGColor;
    }
    if ([dic[@"order_bid_status_id"] intValue] == 140) {
        isSend = NO;
        self.detailBtn.selected = YES;
        [_detailBtn setTitle:@"包裹详情" forState:UIControlStateSelected];
        self.detailBtn.backgroundColor = [Unity getColor:@"aa112d"];
        self.oldBtn.selected = YES;
        self.oldBtn.layer.borderColor = [Unity getColor:@"aa112d"].CGColor;
    }
