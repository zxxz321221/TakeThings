...
@interface HaitaoScreenViewController ()<LMJDropdownMenuDataSource,LMJDropdownMenuDelegate>
{
    UIButton * _seleteBtn;//被选中的button
    NSInteger btnIndex;//筛选按钮单选 记录
    NSInteger listIndex;//列表状态记录
    NSInteger statusIndex;//包裹状态记录
...
@implementation HaitaoScreenViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    _menu1OptionTitles = @[@"询  价  中",@"已　支  付",@"已　结　束"];
    listIndex = self.tap;
    statusIndex = self.statusP;
    btnIndex = self.timeP;
    
    if (listIndex == 0) {
        _menu2OptionTitles = @[@"　全　部　",@"委托审核中",@"报价待支付"];
    }else if (listIndex == 1){
        _menu2OptionTitles = @[@"　全　部　",@"订单已支付",@"订单已采购",@"订单运输中",@"海外已收货"];
    }else{
        _menu2OptionTitles = @[@"　全　部　",@"订单已退款",@"委托已失效",@"委托已取消",@"包裹已签收"];
    }
...
- (UILabel *)line0{
    if (!_line0) {
        _line0 = [[UILabel alloc]initWithFrame:CGRectMake(0, _overTimeL.bottom+[Unity countcoordinatesH:25], SCREEN_WIDTH, 1)];
...
- (UILabel *)precelL{
    if (!_precelL) {
        _precelL = [[UILabel alloc]initWithFrame:CGRectMake([Unity countcoordinatesW:20], _subTitle.bottom+[Unity countcoordinatesW:15], [Unity countcoordinatesW:100], [Unity countcoordinatesH:20])];
        _precelL.text = @"按海淘列表筛选";
...
- (void)createMenu1:(NSArray *)arr{
    ...
    [menu1 setFrame:CGRectMake([Unity countcoordinatesW:120], _precelL.top, [Unity countcoordinatesW:100], [Unity countcoordinatesH:20])];
...
- (void)createMenu2:(NSArray *)arr{
    ...
    [menu2 setFrame:CGRectMake([Unity countcoordinatesW:120], _orderL.top, [Unity countcoordinatesW:100], [Unity countcoordinatesH:20])];
...
- (void)resetClick{
    ...
    if (self.tap == 0) {
        _menu2OptionTitles = @[@"　全　部　",@"订单审核中",@"报价待支付"];
    }else if (self.tap == 1){
        _menu2OptionTitles = @[@"　全　部　",@"订单已支付",@"订单已采购",@"订单运输中",@"海外已收货"];
    }else{
        _menu2OptionTitles = @[@"　全　部　",@"订单已退款",@"委托已失效",@"委托已取消",@"包裹已签收"];
 ...
 */
- (void)confirmClick{
    [self.delegate screenTime:btnIndex WithListIndex:listIndex WithStatusIndex:statusIndex];
    [self exitClick];
}
/**
 时间排序
