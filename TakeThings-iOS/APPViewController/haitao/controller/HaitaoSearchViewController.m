...
@interface HaitaoSearchViewController ()
...
@implementation HaitaoSearchViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [Unity getColor:@"aa112d"];
        self.listArray = [[[NSUserDefaults standardUserDefaults] objectForKey:@"haitaoSearch"] mutableCopy];
        [self createUI];
...
- (UITextField *)searchText{
    if (_searchText == nil) {
        ...
        _searchText.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"直接搜索海淘委托单编号" attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13],NSForegroundColorAttributeName:[Unity getColor:@"#d58896"]}]; ///新的实现
    }
    return _searchText;
}
...
//    NSLog(@"%@",[self.listArray class]);
    ...
    [[NSUserDefaults standardUserDefaults] setObject:self.listArray forKey:@"haitaoSearch"];
}
- (void)searchClick{
...
- (void)push:(NSString *)keyWord{
    HaitaoSearchListViewController * svc = [[HaitaoSearchListViewController alloc]init];
    svc.keyWord = keyWord;
