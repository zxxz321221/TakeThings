...
@interface UsDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,AuctionCellDelegate,GoodsDetailCellDelegate,UsSellerCellDelegate,SDCycleScrollViewDelegate>
{
    NSInteger index;
    NSString * endTime;
    BOOL isAction;
...
//    }
        ...
        return [Unity countcoordinatesH:86];
    }else{
        return [GoodsWebCell cellHeight];
//        _webHeight;//web高度，全局属性
    }
}
...
//    }
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        [cell configWithEndTime:endTime WithPlatform:self.platform WithPrice:[Unity getSmallestUnitOfBid:self.dict[@"goods"][@"Item"][@"CurrentPrice"] WithCount:self.dict[@"goods"][@"Item"][@"BidCount"]] WithBidCount:@""];
        return cell;
    }else if(indexPath.section == 2){
        ...
        UsSellerCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([UsSellerCell class])];
        if (cell == nil) {
            cell = [[UsSellerCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([UsSellerCell class])];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        if (isAction) {
            if (index ==0) {
                cell.htmlString = [NSString stringWithFormat:@"<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Insert title here</title></head><body>%@</body></html>",webUrl];;
            }else{
                cell.htmlString = [NSString stringWithFormat:@"<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Insert title here</title></head><body>%@</body><script type=\"text/javascript\">var head = document.head || document.getElementsByTagName('head')[0];var script4 = document.createElement('div');script4.id = \"ytWidget\";script4.style.display = \"none\";document.body.appendChild(script4);var script5 = document.createElement('script');script5.src = \"https://translate.yandex.net/website-widget/v1/widget.js?widgetId=ytWidget&pageLang=auto&widgetTheme=light&autoMode=true\";head.appendChild(script5);</script></html>",webUrl];
            }
...
//    NSLog(@"footerView 事件（收藏）");
    ...
    if (isGoods) {
        ...
        NSDictionary * dic = @{@"customer":[userInfo objectForKey:@"member_id"],@"area":@"1",@"type":@"goods",@"w_object":self.dict[@"goods"][@"Item"][@"Title"],@"w_link":self.dict[@"goods"][@"Item"][@"ViewItemURLForNaturalSearch"],@"w_overtime":self.dict[@"goods"][@"Item"][@"EndTime"],@"w_jpnid":self.dict[@"goods"][@"Item"][@"ItemID"],@"w_imgsrc":self.dict[@"goods"][@"Item"][@"PictureURL"][0],@"w_saler":self.dict[@"goods"][@"Item"][@"Seller"][@"UserID"],@"w_tag":@""};
        [self.aAnimation startAround];
...
- (void)calculaClick{
    ...
    tvc.price = self.dict[@"goods"][@"Item"][@"CurrentPrice"];
    tvc.goodsTitle = self.dict[@"goods"][@"Item"][@"Title"];
    tvc.imageUrl = self.imagesURLStrings[0];
    tvc.endTime = endTime;
    tvc.increment = [Unity getSmallestUnitOfBid:self.dict[@"goods"][@"Item"][@"CurrentPrice"] WithCount:self.dict[@"goods"][@"Item"][@"BidCount"]];
    tvc.goodsID = self.dict[@"goods"][@"Item"][@"ItemID"];
    tvc.bidorbuy = @"";
    tvc.isDetail = YES;
    tvc.link = self.dict[@"goods"][@"Item"][@"ViewItemURLForNaturalSearch"];
...
- (void)biddingClick{
    ...
    evc.price = self.dict[@"goods"][@"Item"][@"CurrentPrice"];
    evc.goodsTitle = self.dict[@"goods"][@"Item"][@"Title"];
    evc.imageUrl = self.imagesURLStrings[0];
    evc.endTime = endTime;
    evc.increment = [Unity getSmallestUnitOfBid:self.dict[@"goods"][@"Item"][@"CurrentPrice"] WithCount:self.dict[@"goods"][@"Item"][@"BidCount"]];
    evc.goodId = self.dict[@"goods"][@"Item"][@"ItemID"];
    evc.bidorbuy = @"";
...
- (void)goodsDetail{
    ...
    self.pView.inStockL.text = [NSString stringWithFormat:@"%@单位（个，片，台...）",self.dict[@"goods"][@"Item"][@"Quantity"]];
    for (int i=0; i<[self.dict[@"goods"][@"Item"][@"ItemSpecifics"][@"NameValueList"] count]; i++) {
        if ([[self.dict[@"goods"][@"Item"][@"ItemSpecifics"][@"NameValueList"][i] objectForKey:@"Name"] isEqualToString:@"All returns accepted"]) {
            if ([[self.dict[@"goods"][@"Item"][@"ItemSpecifics"][@"NameValueList"][i] objectForKey:@"Value"] isEqualToString:@"Returns Accepted"]) {
...
//    }
    self.pView.bidIdL.text = self.dict[@"goods"][@"Item"][@"ItemID"];
...
//    NSLog(@"SellerCell事件（原始页面）");
    ...
    wvc.webUrl = self.dict[@"goods"][@"Item"][@"ViewItemURLForNaturalSearch"];
    [self.navigationController pushViewController:wvc animated:YES];
}
- (void)allGoodsClick{
//    NSLog(@"SellerCell事件（全部商品）");
    if (!isAction) {
        return;
    }
    ...
    gvc.sellerId = self.dict[@"goods"][@"Item"][@"Seller"][@"UserID"];
...
//    NSLog(@"---------11%@",dic);
    NSArray * allkeys = [dic[@"goods"] allKeys];
    int count=0;
...
//    }
    ...
    NSString * time = dic[@"goods"][@"Item"][@"EndTime"];
...
//    NSLog(@"array1:%@",array10);
    endTime = [NSString stringWithFormat:@"%@/%@/%@",array10[0],array10[1],array10[2]];

//    for (int i=0; i<2; i++) {
//        [self.webArr addObject:dic[@"goods"][@"Item"][@"Description"]];
//    }
    webUrl = dic[@"goods"][@"Item"][@"Description"];
    [self.tableView reloadData];
}
...
- (void)requestIsCollection{
    ...
    NSDictionary * dic = @{@"customer":[userInfo objectForKey:@"member_id"],@"area":@"1",@"item":self.dict[@"goods"][@"Item"][@"ItemID"],@"saler":self.dict[@"goods"][@"Item"][@"Seller"][@"UserID"]};
    NSLog(@"请求参数1 %@",dic);
...
- (void)sellerCollectionClick{
    ...
    if (isSaler) {
        //删除收藏
        NSDictionary * dic = @{@"customer":[userInfo objectForKey:@"member_id"],@"type":@"saler",@"ids":salerStr};
        [self.aAnimation startAround];
        [GZMrequest postWithURLString:[GZMUrl get_deleteCollection_url] parameters:dic success:^(NSDictionary *data) {
            [self.aAnimation stopAround];
            if ([[data objectForKey:@"code"]isEqualToString:@"success"]) {
        ...
        NSDictionary * dic = @{@"customer":[userInfo objectForKey:@"member_id"],@"area":@"1",@"type":@"saler",@"w_object":self.dict[@"goods"][@"Item"][@"Title"],@"w_link":self.dict[@"goods"][@"Item"][@"ViewItemURLForNaturalSearch"],@"w_overtime":self.dict[@"goods"][@"Item"][@"EndTime"],@"w_jpnid":self.dict[@"goods"][@"Item"][@"ItemID"],@"w_imgsrc":self.dict[@"goods"][@"Item"][@"PictureURL"][0],@"w_saler":self.dict[@"goods"][@"Item"][@"Seller"][@"UserID"],@"w_tag":@""};
        NSLog(@"收藏请求= %@",dic);
        [self.aAnimation startAround];
        [GZMrequest postWithURLString:[GZMUrl get_collection_url] parameters:dic success:^(NSDictionary *data) {
            [self.aAnimation stopAround];
