...
@interface EbayGoodsListViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,YhLeftViewDelegate>
{
...
@property (nonatomic , strong) EbayScreenView * sView;
...
@implementation EbayGoodsListViewController
...
//        [cell config:self.goodsList[indexPath.row] WithPlatform:self.platform];
        ...
        cell.imageUrl = self.goodsList[indexPath.row][@"Image"][@"0"];
        cell.goodTitle = self.goodsList[indexPath.row][@"Title"][@"0"];
        cell.currPlace = self.goodsList[indexPath.row][@"CurrentPrice"][@"0"];
    ...
    switch (panGestureRecognizer.state) {
        case UIGestureRecognizerStateChanged:
        {
            ...
            if (panGestureRecognizer.view.center.x + translation.x >= [Unity countcoordinatesW:20] && panGestureRecognizer.view.center.x + translation.x <= self.bigMaxView.center.x-[Unity countcoordinatesW:40]) {
                ...
                if (index == 34) {
                    ...
                    [self.minIndicateView setTitle:[NSString stringWithFormat:@"%@美元",self.placeArray[index]]];
                }
                [self loadPro];
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if ([_minIndicateView.indicateLabel.text isEqualToString:@"未设置"]) {
                ...
                min = [_minIndicateView.indicateLabel.text stringByReplacingOccurrencesOfString:@"美元" withString:@""];
            }
            page=1;
            if (self.isSearch) {
                [self requestGoods];
    ...
    switch (panGestureRecognizer.state) {
        case UIGestureRecognizerStateChanged:
        {
            ...
            if (panGestureRecognizer.view.center.x + translation.x >= self.bigMinView.center.x+[Unity countcoordinatesW:40] && panGestureRecognizer.view.center.x + translation.x <= [Unity countcoordinatesW:300]) {
                ...
                if (index == 34) {
                    ...
                    [self.maxIndicateView setTitle:[NSString stringWithFormat:@"%@美元",self.placeArray[index]]];
                }
                [self loadPro];
                if (self.bigMaxView.left-self.bigMinView.right<=[Unity countcoordinatesW:30]) {
                    self.minIndicateView.hidden = YES;
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if ([_maxIndicateView.indicateLabel.text isEqualToString:@"未设置"]) {
                ...
                max = [_maxIndicateView.indicateLabel.text stringByReplacingOccurrencesOfString:@"美元" withString:@""];
            }
            page=1;
            if (self.isSearch) {
                [self requestGoods];
...
- (void)loadMin:(CGFloat)min{
    ...
    [self.minIndicateView setTitle:[NSString stringWithFormat:@"%.f美元",min]];
    [self loadPro];
    if (self.bigMaxView.left-self.bigMinView.right<=[Unity countcoordinatesW:30]) {
        self.minIndicateView.hidden = YES;
        self.maxIndicateView.hidden = YES;
...
- (void)loadMax:(CGFloat)max{
    ...
    if (max ==0) {
        ...
        [self.maxIndicateView setTitle:[NSString stringWithFormat:@"%.f美元",max]];
    }
...
- (void)loadMaxLabel{
    ...
    if ([self.maxIndicateView.indicateLabel.text isEqualToString:@"未设置"]) {
        ...
        self.mxLabelView.indicateLabel.text = [NSString stringWithFormat:@"%@~%@",[_minIndicateView.indicateLabel.text stringByReplacingOccurrencesOfString:@"美元" withString:@""],self.maxIndicateView.indicateLabel.text];
    }
...
- (NSArray *)placeArray{
    if (!_placeArray) {
        _placeArray = @[@"0",@"5",@"10",@"15",@"20",@"25",@"30",@"35",@"40",@"45",@"50",@"60",@"70",@"80",@"90",@"100",@"150",@"200",@"250",@"300",@"350",@"400",@"450",@"500",@"550",@"600",@"650",@"700",@"750",@"800",@"850",@"900",@"950",@"1000",@"未设置"];
    }
    return _placeArray;
}
- (UIButton *)defaultBtn{
...
//    return _auctionBtn;
//}
//- (EbayScreenView *)sView{
//    if (!_sView) {
//        UIWindow * window = [UIApplication sharedApplication].windows[0];
//        _sView = [EbayScreenView setEbayScreenView:window];
...
- (void)requestGoods{
    ...
    NSDictionary * params = @{@"platform":self.platform,@"locale":self.locale,@"customer":customer,@"word":self.keyWord,@"page":[NSString stringWithFormat:@"%ld",(long)page],@"sort":type,@"buynow":buynow,@"min":min,@"max":max,@"istatus":istatus,@"pstagefree":pstagefree,@"saler":self.sellerId,@"num":self.classId};
...
- (void)requestGoods1{
    ...
    NSDictionary * params = @{@"platform":self.platform,@"num":self.classId,@"page":[NSString stringWithFormat:@"%ld",(long)page],@"sort":type,@"buynow":buynow,@"min":min,@"max":max,@"istatus":istatus,@"pstagefree":pstagefree,@"brand":self.brandId,@"saler":self.sellerId,@"special":self.zhutiId};
    params = [Unity deleteNullValue:params];
    NSLog(@"处理过后%@",params);
...
//            }
            ...
            [[NSUserDefaults standardUserDefaults] setObject:@"1" forKey:@"frist"];
//            [self tableViewLoad:self.goodsList];
            [[JYTimerUtil sharedInstance] timerStart];
        }else{
            [self.altView showAlertView];
            self.altView.msgL.text = [data objectForKey:@"msg"];
...
//    if ([placemax isEqualToString:@""] || [placemin isEqualToString:@""]) {
//        min = @"";
//        max = @"";
//    }else{
//        min = placemin;
...
//    }
//}
//- (void)tableViewLoad:(NSMutableArray *)arr{
//    [self.collectionView reloadData];
...
- (void)cellLongPress:(UILongPressGestureRecognizer *)lpGR{
    ...
    if (lpGR.state == UIGestureRecognizerStateBegan) {//手势开始
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"是否收藏" message:nil delegate:self cancelButtonTitle:@"确认" otherButtonTitles:@"取消", nil];
        [alert show];
    }
}
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex ==0) {
        ...
        NSDictionary * dic = @{@"customer":[userInfo objectForKey:@"member_id"],@"area":@"1",@"type":@"goods",@"w_main_category_id":@"",@"w_goods_category_id":@"",@"w_object":self.goodsList[caIndex.row][@"Title"][@"0"],@"w_link":self.goodsList[caIndex.row][@"ItemUrl"],@"w_overtime":timeStr,@"w_jpnid":self.goodsList[caIndex.row][@"AuctionID"][@"0"],@"w_imgsrc":self.goodsList[caIndex.row][@"Image"][@"0"],@"w_tag":@""};
//        NSLog(@"收藏请求= %@",dic);
        [self.aAnimation startAround];
