...
+(instancetype)setScreenView:(UIView *)view{
    ScreenView * sView = [[ScreenView alloc]initWithFrame:view.frame];
...
- (UIView *)backView{
    if (!_backView) {
        ...
        NSArray * arr = @[@"默认",@"限时购",@"全新",@"免当地运费",@"一口价",@"二手"];
        for (int i=0; i<6; i++) {
            UIButton * btn = [[UIButton alloc]initWithFrame:CGRectMake((i/3+1)*[Unity countcoordinatesW:10]+(i/3)*((_backView.width-[Unity countcoordinatesW:30])/2),self.options.bottom+[Unity countcoordinatesH:10]+(i%3+1)*[Unity countcoordinatesH:10]+(i%3)*[Unity countcoordinatesH:40], (_backView.width-[Unity countcoordinatesW:30])/2, [Unity countcoordinatesH:40])];
