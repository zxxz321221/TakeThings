...
@interface NewOrderViewController ()<UIScrollViewDelegate,ThreeOrderViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    [self.navigationItem setTitle:@"我的委托单"];
...
- (void)setupChildViewController {
    ...
    OneOrderViewController *oneVC = [[OneOrderViewController alloc] init];
    [self addChildViewController:oneVC];
    
    TwoOrderViewController *twoVC = [[TwoOrderViewController alloc] init];
    [self addChildViewController:twoVC];
    
    ThreeOrderViewController * sVC = [[ThreeOrderViewController alloc]init];
    sVC.delegate = self;
...
- (UIButton *)bidBtn{
    if (!_bidBtn) {
        ...
        [_bidBtn setTitle:@"已得标" forState:UIControlStateNormal];
        ...
        CGFloat w = [Unity widthOfString:@"已得标" OfFontSize:15 OfHeight:2];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self addRightBarButtonItemWithTitle:@"取消" action:@selector(cancel)];
...
- (void)addRightBarButtonItemWithTitle:(NSString *)itemTitle action:(SEL)action{
    ...
    self.cancelBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 88, 44)];
    [self.cancelBtn setTitle:itemTitle forState:(UIControlStateNormal)];
    [self.cancelBtn setTitleColor:[UIColor blackColor] forState:(UIControlStateNormal)];
    self.cancelBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    [self.cancelBtn addTarget:self action:action forControlEvents:(UIControlEventTouchUpInside)];
    self.cancelBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    [self.cancelBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 5 * SCREEN_WIDTH/375.0)];
}
//右侧两个图片item的情况
...
{
    _view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 80, 44)];
    ...
    [_view addSubview:firstButton];
    
    UIButton * switchBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    switchBtn.frame = CGRectMake(0, 0, 40, 44);
    [switchBtn setImage:secondImage forState:UIControlStateNormal];
    [switchBtn setImage:[UIImage imageNamed:@"listg"] forState:UIControlStateSelected];
    [switchBtn addTarget:self action:secondAction forControlEvents:UIControlEventTouchUpInside];
    switchBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    [switchBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, - 15 * SCREEN_WIDTH/375.0)];
    [_view addSubview:switchBtn];
    
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_view];
 ...
 */
- (void)searchClick{
    OrderSearchViewController * pvc = [[OrderSearchViewController alloc]init];
...
- (void)screenTime:(NSInteger)time WithListIndex:(NSInteger)listIndex WithStatusIndex:(NSInteger)statusIndex WithType:(NSInteger)type{
    if (listIndex != self.tap) {
        [self topTitleColor:listIndex];
        [self topScrollView:listIndex];
    }
    if (listIndex == 0) {
        oneIndex = time;
        oneStatus = statusIndex;
        oneType = type;
    }else if (listIndex == 1){
        twoIndex = time;
        twoStatus = statusIndex;
        twoType = type;
    }else{
        threeIndex = time;
        threeStatus = statusIndex;
        threeType = type;
    }
    NSDictionary * dic = @{@"time":[NSString stringWithFormat:@"%ld",(long)time],@"status":[NSString stringWithFormat:@"%ld",(long)statusIndex],@"type":[NSString stringWithFormat:@"%ld",(long)type]};
    NSString * str = [NSString stringWithFormat:@"OrderlList%ld",(long)listIndex];
    NSLog(@"%@",str);
    [[NSNotificationCenter defaultCenter] postNotificationName:str object:dic];
}
#pragma mark --threeOrderDelegate--
- (void)confirmDelete{
...
- (void)cancel{
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:_view];
    self.navigationItem.rightBarButtonItem = rightBarButtonItem;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelDelete" object:nil];
}
- (void)cancelDelete{
    [self cancel];
}
