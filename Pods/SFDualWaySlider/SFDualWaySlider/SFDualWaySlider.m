...
- (void)sliderMinValue:(CGFloat)x{
    [_minButton mas_updateConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_progressView.mas_left).offset((self.frame.size.width-40)/105000*x);
    }];
    [_minIndicateView setTitle:[NSString stringWithFormat:@"%.1f万",x/10000]];
...
- (void)sliderMaxValue:(CGFloat)x{
    CGFloat y = _progressWidth/105000*(105000-x);
    NSLog(@"kuandu%f",y);
    [_maxButton mas_updateConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_progressView.mas_right).offset(-y);
    }];
    [_maxIndicateView setTitle:[NSString stringWithFormat:@"%.1f万",x/10000]];
